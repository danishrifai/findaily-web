document.addEventListener("DOMContentLoaded", function () {
  ...
  let saldoAwal = parseInt(localStorage.getItem("saldoAwal")) || 10000000;
  ...
  function updateSaldo() {
    ...
    saldoAmount.textContent = saldoVisible
  ...
  function renderListTransaksi() {
    ...
    dataTransaksi.forEach((trx, index) => {
      ...
      li.innerHTML = `
        ...
        <span class="nominal">${
          trx.jenis === "pemasukan" ? "+" : "-"
        } Rp. ${trx.nominal.toLocaleString("id-ID")}</span>
      `;
  ...
  toggleEye.addEventListener("click", () => {
    ...
    localStorage.setItem("saldoVisible", saldoVisible);
  ...
  function ubahSaldoAwal() {
    ...
    if (input !== null && !isNaN(parseInt(input))) {
      ...
      updateSaldo();
      alert(
        "Saldo awal berhasil diubah menjadi Rp. " +
  ...
  document
    .getElementById("ubahSaldoBtn")
    .addEventListener("click", ubahSaldoAwal);
